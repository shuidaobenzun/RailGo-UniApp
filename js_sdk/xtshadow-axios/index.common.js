...
function wxRequest(config) {
  ...
  var otherConfig = __rest(config, ["url", "method", "data", "header", "responseType", "cancelToken", "validateStatus"]);

  return new Promise(function (resolve, reject) {
    // @ts-ignore
    var reqcall = uni || wx || my || swan || tt || qq;
    var requestTask = reqcall.request(Object.assign(Object.assign({
      url: url,
      header: header,
      method: method,
      data: data,
      ...
      fail: function fail(err) {
        reject(createError(err.errMsg || "Request Fail:" + config.url, 1001, config));
      }
    })); // 请求取消
...
function mergeConfig(defaultConfig, userConfig) {
  var config = Object.create({}); // 创建空对象，作为最终的合并结果
...
Axios.prototype.get = function get(url, data, config) {
  return this._requestMethodWithData(Method.GET, url, data, config);
};
